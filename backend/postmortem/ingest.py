...
def build_store(raw_log: str, llm) -> tuple:
    ...
    error_doc    = Document(
        page_content=f"Major errors found in log:\n{error_lines or 'None detected'}",
    ...
    store = FAISS.from_documents(all_docs, _embeddings)
    print(f"  FAISS store ready. {store.index.ntotal} vectors.\n")

    return store, error_counts# Location: backend/postmortem/ingest.py
import re
from pathlib import Path
