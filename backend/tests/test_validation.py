...
# Skip all validation tests if no real Groq API key is set.
# These tests make real LLM API calls — not suitable for CI without a key.
pytestmark = pytest.mark.skipif(
    not _HAS_REAL_GROQ_KEY,
    reason="GROQ_API_KEY not set — validation tests require real LLM calls",
)
...
def cleanup_faiss():
    yield
    # Remove FAISS stores created during validation tests
    import shutil
    from pathlib import Path
...
def run_pipeline(log_filename: str) -> dict:
    """Run the full postmortem pipeline on a log file and return the state."""
    import time
    ...
    # Use dummy user_id and token — we don't need DB or FAISS persistence for validation
    last_error = None
...
class TestGitLab2017:
    ...
    def test_root_cause_identifies_human_error(self, result):
        ...
        # The root cause was human/operator error — accidental deletion
...
class TestAWS2020:
    ...
    def test_timeline_shows_extended_duration(self, result):
        report = result["report_str"].lower()
        # AWS 2020 lasted ~5 hours — should mention extended duration
